...
def main():
    ...
    with open(params_filepath) as params_file:
        ...
        __plot_occupancy_vs_mean(input_folder)
        __plot_occupancy_vs_std(input_folder)
        __print_latex_table(input_folder, "pileup_occupancy")


def __plot_occupancy_vs_mean(folder):
    output_file = "occupancy_vs_mean.pdf"
    x_field = "pileup_occupancy"
    x_label = "Ocupação [%]"
    y_field = "mean"
    y_label = "Média do Erro de Estimação [MeV]"
    plot_summary(folder, output_file, x_field, x_label, y_field, y_label)

def __plot_occupancy_vs_std(folder):
    output_file = "occupancy_vs_std.pdf"
    x_field = "pileup_occupancy"
    x_label = "Ocupação [%]"
    y_field = "std"
...
def __plot_histograms(folder, params):
    def __title_builder(job):
        pileup_occupancy = job["pileup_occupancy"]
        return f"Filters Efficiency With Occupancy = {pileup_occupancy * 100}%"
